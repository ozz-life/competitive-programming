...
{
    visited[y][x] = 1; // помечаем текущую клетку, как посещённую
    ...
    // static vector<int> dy = {-1, 0, 1, 0, -1, -1, 1, 1}; // добавляем северо-запад, северо-восток, юго-восток и юго-запад
    ...
    for (int d = 0; d < dy.size(); ++d)
    {
        ...
        if (0 <= ty && ty < a.size() && // Проверить не уйдёт ли сосед за границу карты
            ...
            !visited[ty][tx])     // Проверить не была ли эта клетка посещена ранее
        {
            dfs(a, ty, tx, visited);
        }
    }
