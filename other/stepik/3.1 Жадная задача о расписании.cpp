...
// У программиста пять заказов
// с дедлайнами d = [1, 2, 2, 3, 5]
//  и стоимостями c = [2, 5, 4, 1, 3].
//  Какую максимальную сумму сможет заработать программист?


// Сортировка сложного объекта заказов
struct Order {
  int d, c;
};
...
void solve() {
  std::vector<int> d; // дедлайны
  std::vector<int> c; // стоимости
  std::vector<bool> used(TMAX, false);

  vector<Order> order;
  sort(order.begin(), order.end());

  int64_t sum = 0;
  // В цикле по заказам (отсортированным в порядке убывания стоимости)
  for (int i = 0; i < c.size(); ++i) {
    int k = d[i]; // номер дня, в который будем выполнять
    while (k >= 1 && used[k])
      k--;
    if (k == 0)
      continue;
