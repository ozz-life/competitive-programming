...
void solve() {
// Разберём ещё одну задачу, решаемую жадным алгоритмом, — задачу о вы-
// боре заявок.
...
// ресекались.
  ...
  // При нуль индексации, мы считаем, что левая граница входит в интервал,
  // а правая нет.

  // Отсортируем заявки по возрастанию правой границы. Идём по ним слева
  // направо. Нам выгоднее брать ту заявку, которая раньше заканчивается, потому
  // что это нам поможет взять какие-то заявки в дальнейшем. Потому берём
  // очередную заявку если она начинается не раньше, чем заканчивается
  // предыдущая взятая.

  int cnt = 1;  // количество удовлетворённых заявок, первая заявка с индексом 0
  // сразу удовлетворяется, потому cnt = 1
  int last = 0; // номер последней удовлетворённой заявки

  // Заявки отсортированы по возрастанию правой границы

  for (int i = 1; i < n; ++i) {
    // Если левая граница заявки l[i] больше или равна последней границе последней
    // взятой заявки, то мы берём i-ю заявку, увеличиваем cnt и last присваиваем
    // значение i - номер последней взятой заявки
    if (l[i] >= r[last]) {
      cnt++;
      last = i;
    }
