...
void printPrefixSum(const std::vector<std::vector<int64_t>> &prefixSum) {
  for (size_t i = 0; i < prefixSum.size(); ++i) {
    std::cout << "Row " << i << ": ";
    for (size_t j = 0; j < prefixSum[i].size(); ++j) {
...
// Первичное прочтение 9 минут.
// 7 мин, надо думать.
// 17-00 st, 2 часа 20 минут первая отправка WA
// 3 часа, 10 WA
// 20 min

void solve() {
	    for (int i = 0; i < 10; ++i) {
        cout << "i = " << i << ": "; // Выводим текущее значение i
        for (int j = 0; j < 10; j++) {
            cout << "j = " << j << " -> " << i * j << "\t"; // Выводим текущее значение j и произведение
        }
