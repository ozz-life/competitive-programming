...
void solve() {
  ...
  std::map<std::string, int> wordToNumber = {
    ...
    int firstValue = -1;  // Инициализируем переменную значением, которое не может быть результатом
    while (std::getline(std::cin, line)) {
        for (char ch : line) {
            if (std::isalpha(static_cast<unsigned char>(ch))) {
                // Если символ буквенный, формируем слово
                std::string word;
                ...
                // Проверяем, является ли слово ключом в map
                auto it = wordToNumber.find(word);
                if (it != wordToNumber.end()) {
                    // Если слово найдено в map, записываем значение и выходим из цикла
                    firstValue = it->second;
                    break;
                }
            } else if (std::isdigit(static_cast<unsigned char>(ch))) {
                // Если символ цифра, записываем значение и выходим из цикла
                firstValue = ch - '0';
                break;
            }
        }

        // Если нашли значение, выходим из внешнего цикла
        if (firstValue != -1) {
            break;
        }
    }

    // Вывести найденное значение
