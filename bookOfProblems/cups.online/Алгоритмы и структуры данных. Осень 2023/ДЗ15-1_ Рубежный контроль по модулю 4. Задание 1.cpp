...
// Дан ориентированный взвешенный граф, представленный в виде списка рёбер.
// Напишите функцию на C++, которая находит кратчайшие пути от заданной
// начальной вершины до всех остальных вершин с использованием алгоритма
// Беллмана-Форда. Функция также должна определять наличие отрицательных циклов
// в графе.
...
public:
  ...
  void BellmanFord(const int startId) {
    ...
    dist[startId] = 0;
    for (unsigned int i = 1; i <= adjacencyList.size() - 1; i++) {
      for (const auto &node : adjacencyList) {
        for (const Edge &edge : node.second) {
          if (dist[node.first] != std::numeric_limits<int>::max() &&
              dist[node.first] + edge.weight < dist[edge.node]) {
    ...
    for (const auto &node : adjacencyList) {
      std::cout << "Distance to node " << node.first << " = "
                << dist[node.first] << std::endl;
    }
  }
};
