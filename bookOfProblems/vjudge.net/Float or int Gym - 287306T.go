...
func solve(in *bufio.Reader, out *bufio.Writer) {
	...
	allZerosAfterPoint := true
	pointIdx := -1

	var integerPart strings.Builder
	var fractionalPart strings.Builder
	fractionalPart.WriteString("0.")
	...
	for i := 0; i < len(s); i++ {
		...
		integerPart.WriteByte(s[i])
	}

	if pointIdx != -1 {
		for i := pointIdx + 1; i < len(s); i++ {
			fractionalPart.WriteByte(s[i])
			if s[i] != '0' {
				allZerosAfterPoint = false
			}
		}
	}

	if allZerosAfterPoint {
