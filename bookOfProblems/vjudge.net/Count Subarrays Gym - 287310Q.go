...
func solve(in *bufio.Reader, out *bufio.Writer) {
	...
	// Правильный обход
	// subbarrays := make([][]int, 0)
	// for start := 0; start < len(a); start++ {
	// 	for end := start; end < len(a); end++ {
	// 		subarray := make([]int, end-start+1)
	// 		copy(subarray, a[start:end+1])
	// 		subbarrays = append(subbarrays, subarray)
	// 	}
	// }

	// Тоже правильный обход
	subbarrays := make([][]int, 0)
	for i := 0; i < len(a); i++ {
		for j := i; j < len(a); j++ {
			subarray := make([]int, 0)
			for k := i; k <= j; k++ {
				subarray = append(subarray, a[k])
			}
			subbarrays = append(subbarrays, subarray)
		}
	}
	...
	var count int
	for _, subarray := range subbarrays {
		isIncreasing := true
