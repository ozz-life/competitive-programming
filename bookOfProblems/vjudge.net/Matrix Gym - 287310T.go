...
func solve(in *bufio.Reader, out *bufio.Writer) {
	...
	for i := 0; i < n; i++ {
		primaryDiagonalSum += matrix[i][i]       // Главная диагональ
		secondaryDiagonalSum += matrix[i][n-i-1] // Побочная диагональ
	}

	absoluteDifference := int(math.Abs(float64(primaryDiagonalSum - secondaryDiagonalSum)))

	// Вывод результата
	// fmt.Fprintln(out, primaryDiagonalSum)
	// fmt.Fprintln(out, secondaryDiagonalSum)
	fmt.Fprintln(out, absoluteDifference)
