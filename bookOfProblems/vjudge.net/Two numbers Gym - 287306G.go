...
func solve(in *bufio.Reader, out *bufio.Writer) {
	...
	ansFloor := math.Floor(result)
	ansCeil := math.Ceil(result)
	ansRound := math.Round(result)

	fmt.Fprintf(out, "floor %.0f / %.0f = %.0f\n", A, B, ansFloor)
	fmt.Fprintf(out, "ceil %.0f / %.0f = %.0f\n", A, B, ansCeil)
	fmt.Fprintf(out, "round %.0f / %.0f = %.0f\n", A, B, ansRound)
}
