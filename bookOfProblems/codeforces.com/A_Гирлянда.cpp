...
void solve()
{
    ...
    // Заметим, что существует всего несколько классов конфигураций: 1111, 1112, 1122, 1123 и 1234. Рассмотрим каждую из них.

    // Если все

    // лампочки одного цвета, то невозможно включить их все, т.к. после включения одной лампочки включить другие невозможно.

    // Если есть цвет с 3
    // лампочками, то включить все лампочки за 4 операции невозможно, а значит придется хотя бы раз какую-то лампочку включить, выключить, а затем опять включить, т.е. ответ хотя бы 6 операций. Существует последовательность ровно из 6

    // операций (такой пример был разобран в примечании к задаче).

    // Для конфигураций вида 1122 и 1123 достаточно включать лампочки цвета 1
    // не подряд (т.е. в порядке [1,2,1,2] для первого случая и [1,2,1,3] для второго). Таким образом ответ для таких конфигураций равен 4

    // .

    // Если все лампочки разных цветов, то ничего не мешает включить их все за 4
    // операции.
    // выбрать лампочку и переключить ее состояние (включить, если была выключена, и выключить, если была включена).
    // Единственное ограничение на вышеописанную операцию — вы можете применять операцию к лампочке, только если предыдущая операция была применена к лампочке другого цвета (первую операцию можно применять к любой лампочке).

    string s;
    cin >> s;

    // vector<int64_t> counter(4);
    // for (auto &c : s)
    // {
    //     ++counter[c - '0'];
    // }

    // int64_t mx = *max_element(all(counter));
    // if (mx == 4)
    //     cout << -1;
    // else if (mx == 3)
    //     cout << 6;
