...
// Let's compute the prefix sums of the array a: let bi=a1+⋯+ai. Rephrasing the
// problem: for each question containing an integer k, we need to find the
// largest ai such that a1,…,ai are all at most k, and then output bi. In other
// words, max(a1,…,ai)≤k

// Let's make the prefix maximums of the array: let mi=max(a1,…,ai)
// . Then we need to find the largest i such that mi≤k, which is doable using
// binary search, since the array m is non-decreasing. Once we find the index i,
// we simply need to output bi
...
void solve() {
  ...
  //     int k;
  //     cin >> k;
  //     int ind = upper_bound(prefmax.begin(), prefmax.end(),
  //     k)-prefmax.begin(); cout << pref[ind] << " ";
  // }
  // cout << endl;
}
...
int32_t main() {
  ...
  // отключения). в интерактивных
  // задачах обязательно использовать endl для очистки буфера.
  std::cin.tie(nullptr);
