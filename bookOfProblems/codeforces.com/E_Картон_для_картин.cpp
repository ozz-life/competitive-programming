...
void solve() {
    ...
    int s = 0; // Сумма всех размеров картин
    for (int i = 0; i < n; ++i) {
        int x;
        cin >> x;
        s += x;
        c -= x * x; // хранит оставшуюся площадь картонa после вычитания квадрата каждого размера картинки.
    }
	...
	while (l + 1 < r) {
		...
		// максимально возможная ширина (c / m / n + 100) 
        //if (m > c / m / n + 100 || n * m * m + s * m > c) {
		if (m > c / (m * n) + 100 || n * m * m + s * m > c) {
            r = m;
        } else {
            l = m;
