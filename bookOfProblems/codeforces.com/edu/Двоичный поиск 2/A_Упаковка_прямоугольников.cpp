...
bool good(int64_t w, int64_t h, int64_t n, int64_t m)
{
    return (m / w) * (m / h) >= n;
}
...
void solve()
{
    // int64_t w, h, n;
    // cin >> w >> h >> n;
    // int64_t l = 0;    // Размер квадрата, в который точно не влезает
    // int64_t r = 1e18; // А сюда должно точно влезть, допустим в 1e18

    // while (l + 1 < r)
    // {
    //     int64_t m = l + (r - l) / 2;

    //     // Сначала неправильное решение, здесь переполнятся лонги, потому что m большое и мы получим два числа порядка 18, перемножим их и это вылезет
    //     // int64_t s = (m / w) * (m / h);

    //     // Способ бороться 1. Делать в даблах
    ...
    // 3 использовать int128
    // 4 использовать питон
}
