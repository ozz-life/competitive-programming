...
void solve() {
  ...
  // Чётное число - even
  // Нечётное число - odd
  // а) сумма двух четных чисел четна;  сумма любого числа четных чисел будет
  // четной. б) сумма двух нечетных чисел четна; в) сумма четного и нечетного
  // чисел нечетное число.
  // Сумма нечетного числа нечетных чисел будет нечетной,
  // а сумма четного числа нечетных чисел будет четной.
  // произведение любого числа четных чисел будет четным.
  ...
  //       odd_cnt++;
  //   }
  //   int even_cnt = a.size() - odd_cnt;

  //   // cout << even_cnt << " " << odd_cnt << "\n";

  //   // Чётное чётных. Чётное нечётных.
  //   if (even_cnt % 2 == 0 and odd_cnt % 2 == 0) {
  //     cout << "YES"
  //          << "\n";
  //   }
  //   // Нечётное чётных. Нечётное нечётных.
  //   if (even_cnt % 2 == 1 and odd_cnt % 2 == 1) {
  //     cout << "NO"
  //          << "\n";
  //   }
  //   // Чётное чётных. Нечётное нечётных
  //   if (even_cnt % 2 == 0 and odd_cnt % 2 == 1) {
  // 	    cout << "NO"
  //          << "\n";
  //   }
  //   // Нечётное чётных. Чётное нечётных
  //     if (even_cnt % 2 == 1 and odd_cnt % 2 == 0) {
  // 	    cout << "YES"
  //          << "\n";
  //   }

  // так как сумма массива нечетная, то вы не сможете разделить его на две части
  // одна будет четной, а другая - нечетной, и если это четное число, вы можете
  // разделить его как угодно, и четность будет сохраняться, потому что общая
  // сумма четная. И так как n составляет не менее двух, мы всегда можем
  // разделить его таким образом, чтобы перекрасить.

  cout << (std::accumulate(a.begin(), a.end(), 0LL) % 2 == 0 ? "YES" : "NO")
       << "\n";
}
