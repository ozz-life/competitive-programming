...
//     // auto index = std::distance(a.rbegin(), it);
//     int index = std::distance(it, a.rend()) - 1;
//     if (it != a.rbegin() && *it >= *(it - 1)) {
//       lastMaxIdx = index;
//       break;
//     }
//   }
	...
	int ans = 0;
	for(int i = 1; i < n; ++i) {
		if(a[i - 1] > a[i]) std::max(ans, a[i - 1]);
	}

  //int mxall = *std::max_element(a.begin(), lastMaxIdx);
