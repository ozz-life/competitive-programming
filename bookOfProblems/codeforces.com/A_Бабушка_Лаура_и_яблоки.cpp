...
void solve() {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    // Отладочный вывод для текущего значения s и apples
    // std::cout << "i=" << i << " s=" << s << " apples_before=" <<
    // std::bitset<64>(apples) << std::endl;

    if (s == "halfplus") {
      apples |= (1LL << i);
    }

    // Отладочный вывод для текущего значения apples после выполнения if
    // std::cout << "i=" << i << " apples_after=" << std::bitset<64>(apples) <<
    // std::endl;
  }

  // std::bitset<64> applesBitset(apples);
  // std::cout << "Apples in binary: " << applesBitset << std::endl;

  int64_t gifts = std::popcount(apples);
  // std::cout << "Number of gifts: " << gifts << std::endl;

  std::cout << apples * p - gifts * p / 2 << std::endl;
}
