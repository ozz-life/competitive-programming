...
// Задача может быть решена одним проходом по массиву. Поддерживаем cur —
// текущий минимум, pos — позиция последнего вхождения cur, ans — текущее
// минимальное расстояние между двумя вхождениями cur. Теперь для каждого i если
// ai < cur, то сделаем cur:  = ai, pos:  = i, ans:  = ∞. Для ai = cur сделаем
// ans = min(ans, i - pos), pos:  = i. Тогда в конце cur будет глобальным
// минимумом в массиве, а ans будет содержать два его ближайших вхождения.
...
void solve() {
  ...
  int64_t current_min = numeric_limits<int64_t>::max();
  int64_t last_occurrence = numeric_limits<int64_t>::max();
  ...
  for (int i = 0; i < n; ++i) {
    if (a[i] < current_min) {
      current_min = a[i];
      last_occurrence = i;
      answer = numeric_limits<int64_t>::max();  // Обнуляем ответ при изменении текущего минимума
    } else if (a[i] == current_min) {
      answer = std::min(answer, i - last_occurrence);
      last_occurrence = i;
    }
  }

  // Если минимальный элемент встречается только один раз, ответ будет 0
