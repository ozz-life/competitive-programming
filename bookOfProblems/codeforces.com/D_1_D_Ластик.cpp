...
//   int N, K; // Объявляем переменные для длины полоски (N) и параметра K.
//   string S; // Объявляем строку S для хранения состояния полоски.
//   cin >> N >> K >> S; // Считываем N, K и строку S.

//   int ops = 0; // Переменная ops будет хранить количество операций.

//   for (int i = 0; i < N; i++) { // Итерируемся по каждой ячейке полоски.
//     if (S[i] == 'B') { // Если текущая ячейка черная ('B').
//       for (int j = i; j < min(i + K, N);
//            j++) { // Итерируемся по следующим K ячейкам или меньшему числу
//                   // ячеек, если достигнем конца полоски.
//         S[i] = 'W'; // Заменяем черные ячейки на белые.
//       }
//       i += K - 1; // Увеличиваем i на K - 1, чтобы пропустить уже обработанные
//                   // черные ячейки.
//       ops++;      // Увеличиваем счетчик операций.
//     }
//   }
