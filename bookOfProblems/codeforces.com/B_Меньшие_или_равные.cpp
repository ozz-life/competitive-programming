...
int bs(const std::vector<int>& a, int k) {
    int l = 1, r = 1e9 + 1;

    while (l < r) {
        int m = l + (r - l) / 2;
        int countLessOrEqual = std::upper_bound(a.begin(), a.end(), m) - a.begin();
    ...
    if (std::upper_bound(a.begin(), a.end(), l) - a.begin() != k) {
        return -1;
    }
