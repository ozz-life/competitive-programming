...
pair<int, vector<int>> findCycle(const vector<Edge> &edges, int n) {
  ...
  while (true) {
    ...
    int nextVertex = -1;
    for (const Edge &edge : edges) {
      if (edge.from == startVertex && !visited[edge.to]) {
        nextVertex = edge.to;
        break;
      }
      if (edge.to == startVertex && !visited[edge.from]) {
...
void solve() {
  ...
  auto [minWeight, cycle] = findCycle(edges, n);
 
  cout << minWeight << " " << cycle.size() << "\n";
