...
// В этой задаче требовалось найти подмножество наименьшего размера, сумма чисел
// в котором строго больше, чем половина суммы всех чисел. Несложно понять, что
// выгоднее брать наибольшие числа в массиве, так мы быстрее наберем нужную
// сумму. Тогда решение выглядит следующим образом: отсортируем числа по
// невозрастанию и переберем сколько первых чисел заберем мы, а сколько – наш
// близнец, Как только у нас строго больше, чем осталось для близнеца – выведем
// ответ.
...
void solve() {
  ...
  for (int i = 0; i < n; ++i) {
    sum_taken += coins[i];
    sum_remaining = std::accumulate(coins.begin() + i + 1, coins.end(), 0);

    if (sum_taken > sum_remaining) {
