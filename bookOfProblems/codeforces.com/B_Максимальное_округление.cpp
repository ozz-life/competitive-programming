...
void solve() {
  ...
  // Добавляем ведущий ноль и переворачиваем число
  s = '0' + s;
  std::reverse(s.begin(), s.end());

  int k = 0; // Отслеживаем разряд.

  for (int i = 0; i < l; ++i) {
    if (s[i] >= '5') {
      s[i + 1]++;
      // Заменяем цифры на нули до текущего разряда
      for (int j = i; j >= k; j--) {
        s[j] = '0';
      }
      k = i;
  ...
  // Опять переворачиваем число и убираем ведущие нули
  std::reverse(s.begin(), s.end());
  int start = 0;
  while (start < l && s[start] == '0') {
    start++;
  }
