...
vector<int> buildPrefixHash(const string& s) {
    int n = s.size();
    vector<int> h(n + 1, 0);
    //int p = 31; // Простое число p (можно выбрать другое)
	int p = 2147483647; // Максимально возможное простое число для int
    int mod = 1000000009; // Используем целое число
    int x = 1;
    
    for (int i = 0; i < n; i++) {
        h[i + 1] = (1LL * h[i] + (s[i] - 'a' + 1) * x) % mod;
        x = (1LL * x * p) % mod;
    }
...
void solve() {
    ...
    int n = s.size();
    vector<int> prefixHash = buildPrefixHash(s);
    
    int q;
    cin >> q;
    
    int mod = 1000000009; // Объявляем и инициализируем mod здесь
    
    for (int i = 0; i < q; i++) {
        int len, a, b;
        cin >> len >> a >> b;
        
        int hash1 = (prefixHash[a + len] - prefixHash[a] + mod) % mod;
        int hash2 = (prefixHash[b + len] - prefixHash[b] + mod) % mod;
        
        if (len == 1 || (hash1 == hash2)) {
            cout << "yes" << endl;
        } else {
            cout << "no" << endl;
        }
