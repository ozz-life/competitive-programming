...
// Обратите внимание, что коробки укладываются в порядке возрастания ИНДЕКСОВ, а
// разность между какими-либо грузовиками должна быть МАКСИМАЛЬНА
...
void solve() {
  ...
  sort(a.begin(), a.end()); // Сортируем по возрастанию
  ...
  // Проходим по каждому делителю
  for (int divisor : divisors) {
    vector<int> sums;

    // Проходим по массиву с шагом делителя
    for (int start = 0; start < divisor; ++start) {
      int sum = 0;
      for (int i = start; i < n; i += divisor) {
        sum += a[i];
      }
      sums.push_back(sum);
    }

    // Находим максимальное и минимальное значение сумм
