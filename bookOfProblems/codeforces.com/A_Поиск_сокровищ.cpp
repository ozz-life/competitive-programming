...
// (x2-x1)%x==0 and (y2-y1)%y==0
// Нам важна только чётность этого числа, если чётное, значит ответ существует

// Это работает, но вы также должны проверить, можно ли достичь точки, делая
// движения в L. Как лошадь на шахматах. Она будет достижима, если количество
// ходов, сделанных по оси x + количество ходов, сделанных по оси y, будет
// четным. Это происходит потому, что когда вы двигаетесь, вы всегда делаете ход
// по обеим осям.
...
void solve() {
  ...
  int needx = dx / x;
  int needy = dy / y;
  if (dx % x == 0 and dy % y == 0 and (needx % 2) == (needy % 2)) {
    cout << "YES\n";
  } else {
    cout << "NO\n";
  }
