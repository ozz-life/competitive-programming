...
void generateBinaryNumbersHelper(int curr, int max, vector<int> &result) {
  if (curr > max) {
    return;
  }
...
vector<int> generateBinaryNumbers(int curr, int max) {
  vector<int> result;
  generateBinaryNumbersHelper(curr, max, result);
  return result;
}

bool checkBinaryProduct(int n) {
  if (n == 1) {
    return true;
  }

  vector<int> generatedNumbers = generateBinaryNumbers(1, n);
  generatedNumbers.push_back(1);
  ...
  for (int i = 0; i < generatedNumbers.size(); i++) {
    for (int j = i; j < generatedNumbers.size(); j++) {
