...
/**
 * @brief Вычисляет сумму всех степеней двойки, считая единицу.
 * На выходе получает число Мерсенна a(n) = 2^n - 1. 
 * https://oeis.org/A000225
 * @param n Порядковый номер максимальной степени двойки в сумме.
 * @return Сумма всех степеней двойки
 */
int64_t sum_of_all_powers_of_two(int64_t n) {
    int64_t sum = 0;
...
void solve() {
  ...
  int64_t arithmeticProgression = n * (n + 1) / 2;

  int64_t powersSum = sum_of_all_powers_of_two(n);
  int64_t answer = arithmeticProgression - (powersSum * 2);
