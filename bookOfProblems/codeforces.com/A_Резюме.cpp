...
// В условии буквально написано найти наидлиннейшую подпоследовательность,
// выглядющую следующим образом: [0, 0, 0, ..., 1, 1, 1]. Давайте найдем,
// сколько в ней будет нулей и соберем все единицы после позиции последнего. На
// каждом шаге будем добавлять очередной ноль с начала строки и считать
// количество единиц после него. Обновляем ответ максимумом.

// Также можно предподсчитать количество единиц на суффиксе с помощью частичных
// сумм, но в данной задаче этого не требовалось.

// Асимптотика решения: O(n2) (наивно) или O(n) (с частичными суммами).
...
//   // Инициализация векторов x и y для подсчета количества нулей и единиц
//   vector<int> x(n + 1, 0); // Вектор для подсчета количества нулей
//   vector<int> y(n + 2, 0); // Вектор для подсчета количества единиц

//   // Считываем значения и обновляем векторы x и y
//   for (int i = 1; i <= n; i++) {
//     int k;
//     cin >> k;
//     if (!k)
...
//   //Кумулятивная сумма для вектора x
//   for (int i = 1; i <= n; i++) {
//     x[i] += x[i - 1];
//   }

//   // Кумулятивная сумма для вектора y в обратном порядке
//   for (int i = n; i >= 1; i--) {
//     y[i] += y[i + 1];
//   }
...
//   // Находим максимальное значение суммы x[i] + y[i + 1]
//   for (int i = 0; i <= n; i++) {
//     ans = std::max(ans, x[i] + y[i + 1]);
//   }
...
//   // перебирает все возможные разделения массива на две части (от 0 до i и от
//   i
//   // до n).
//   for (int i = 0; i <= n; ++i) {
//     int now = 0;

//     // Считаем количество нулей в левой части разделения
//     for (int j = 0; j < i; ++j) {
//       if (!a[j]) {
//         ++now;
//       }
...
//     // Считаем количество единиц в правой части разделения
//     for (int j = i; j < n; ++j) {
//       if (a[j]) {
//         ++now;
//       }
...
void solve() {
  ...
  for (int i = 0; i <= n; ++i) {
    ...
    std::cout << "Partition at " << i << ": Zeros in left: " << now
              << std::endl;

    ans = std::max(ans, now);
  }
  std::cout << "Maximum ones after partition: " << ans << "\n";
}
