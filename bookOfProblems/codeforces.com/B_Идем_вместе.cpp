...
void solve()
{
    ...
    // // Вычисляются значения переменных AB и AC, которые представляют сумму модулей разностей координат точек AB и AC соответственно. Формула AB = abs(Ax - Bx) + abs(Ay - By) вычисляет сумму разностей по осям x и y между точками A и B. Аналогично, формула AC = abs(Ax - Cx) + abs(Ay - Cy) вычисляет сумму разностей по осям x и y между точками A и C.
    ...
    // cout << abs(xa - med(xa, xb, xc)) + abs(ya - med(ya, yb, yc)) + 1 << '\n';
    ...
    //     for (int i = 0; i < 2; i++)
    //     {
    //         int x = a[1][i] - a[0][i], y = a[2][i] - a[0][i];
    //         if (x < 0)
    //         {
    //             x *= -1;
    //             y *= -1;
