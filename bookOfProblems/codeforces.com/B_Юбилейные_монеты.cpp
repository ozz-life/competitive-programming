...
//   int64_t t =
//       std::max<int64_t>(0, std::min<int64_t>(m / k, (m - a1 + k - 1) / k));
//   int64_t ans =
//       std::max<int64_t>(0, t - ak) + std::max<int64_t>(0, m - t * k - a1);
	  
//   int t =
//       std::max<int>(0, std::min<int>(m / k, (m - a1 + k - 1) / k));
//   int ans =
//       std::max<int>(0, t - ak) + std::max<int>(0, m - t * k - a1);

  int t =
      std::max(0, std::min(m / k, (m - a1 + k - 1) / k));
  int ans =
      std::max(0, t - ak) + std::max(0, m - t * k - a1);
