...
void solve() {
  // n — количество людей
  // m — количество ступенек
  // k — разница высот между соседними ступеньками
  // H — рост Влада.
  int n, m, k, H;
  cin >> n >> m >> k >> H;
  std::vector<int> a(n);
  ...
  int ans = 0;
  for (auto &i : a) {
    if (std::abs(H - i) <= range)
      ans++;
  }
  cout << ans << "\n";

  ///////////
  std::vector<vector<int>> d((1 << n), vector<int>(n, INF));
  d[0][0] = 0;
  for (int mask = 0; mask < (1 << n); ++mask) {
    for (int i = 0; i < n; ++i) {
      ...
      for (int j = 0; j < n; ++j) {
        if (!(mask & (1 << j))) {
          d[mask ^ (1 << j)][j] =
              std::min(d[mask ^ (1 << j)][j], d[mask][i] + a[i][j]);
        }
      }
    }
  }
  
  cout << d[(1 << n) - 1][0] << std::endl;
}
