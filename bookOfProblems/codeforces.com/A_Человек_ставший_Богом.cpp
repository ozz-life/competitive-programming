...
int64_t calculateForce(const vector<int64_t>& arr, int64_t left, int64_t right) {
    int64_t force = 0;
    for (int64_t i = left; i < right; i++) {
        force += abs(arr[i] - arr[i + 1]);
    }
    return force;
}

int64_t findMinimumForce(const vector<int64_t>& arr, int64_t n, int64_t k) {
    int64_t minForce = INT_MAX;

    for (int64_t i = 0; i <= n - k; i++) {
        for (int64_t j = i + k - 1; j < n; j++) {
            int64_t force = calculateForce(arr, i, j);
            minForce = min(minForce, force);
        }
    }
...
{
    int64_t n, k;
    cin >> n >> k; // количество жителей, на сколько разбить
    ...
    int64_t minimumForce = findMinimumForce(arr, n, k);
    cout << minimumForce << endl;
}
