...
void solve() {
    ...
    // Ввод массива b и одновременно вычисление массива a с учетом вычитания элементов b
    for (int i = 0; i < n; ++i) {
        std::cin >> b[i];
        a[i] -= b[i];
    }
    ...
    // Считаем, сколько раз в массиве a встречается максимальное значение
    int countOfMax = std::count(a, a + n, ma);
    
    // Выводим количество сильных вершин
    std::cout << countOfMax << '\n';
    
    // Выводим индексы сильных вершин
    for (int i = 0; i < n; ++i) {
        if (a[i] == ma) {
            std::cout << i + 1 << ' ';
        }
