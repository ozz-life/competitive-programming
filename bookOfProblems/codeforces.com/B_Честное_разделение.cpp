...
void solve() {
    ...
    // Считываем веса конфет и подсчитываем количество конфет веса 1 и 2
    for (int i = 0; i < n; i++) {
        int c;
        std::cin >> c;
        if (c == 1) {
    ...
    // Если сумма весов нечетная
    if (isOdd(cnt1 + 2 * cnt2)) {
        std::cout << "NO\n";
    } else {
        int sum = (cnt1 + 2 * cnt2) / 2;

        // Если сумма четная, или сумма нечетная, но есть конфеты веса 1, выводим "YES"
