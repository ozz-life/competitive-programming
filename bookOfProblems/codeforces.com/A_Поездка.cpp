...
int maxDifferenceBetweenNeighbors(const std::vector<int>& arr) {
    if (arr.size() < 2) {
        // If the array has less than two elements, there are no neighbors.
...
int maxDiff(std::vector<int> &arr, int n) {
...
void solve() {
  ...
  cout << maxDifferenceBetweenNeighbors(a);
}
