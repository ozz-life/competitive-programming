...
void solve()
{
    int numOfApps, numOfQrs, windowSize;
    cin >> numOfApps >> numOfQrs >> windowSize;

    vector<int> order(numOfApps), indexOf(numOfApps + 1);
    ...
    for(int i = 0; i < numOfQrs; ++i) {
        ...
        ans += indexOf[app] / windowSize + 1;
        int indOfApp = indexOf[app];
        if ( indOfApp > 0 ) {
            int indOfPrev = indOfApp - 1;
            int prevApp = order[indOfPrev];
            std::swap(order[indOfApp], order[indOfPrev]);
            std::swap(indexOf[app], indexOf[prevApp]);
        }
    }
