...
// Для начала научимся определять, сколько времени должно пройти, чтобы сработал
// i-й будильник. Если время будильника больше текущего, то очевидно должно
// пройти 60∗(hi−H)+mi−M минут. Иначе эта величина будет отрицательной и тогда
// должно пройти 24∗60+60∗(hi−H)+mi−M

// так как должен пройти полный день, не включая время от будильника до текущего
// времени. Нам нужно лишь найти минимальное количество минут среди всех
// будильников.

// Из-за маленьких ограничений задачу также можно решить симулируя процесс сна:
// увеличивать ответ на 1 и проверять, не сработает ли будильник через столько
// времени.
