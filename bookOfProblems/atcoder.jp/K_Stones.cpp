...
//   vector<bool> dp(k + 1);
//   for (int i = 1; i <= k; i++) { // фиксируем состояние
//     for (int j = 0; j < n; j++) { // фиксируем переход в него
//       if (a[j] > i) {
//         continue;
//       }
//       if (!dp[i - a[j]]) { // реализуем переход
//         dp[i] = 1;
//       }
//     }
//   }
...
//     // DP массив, где dp[i] равен true, если текущий игрок может победить при i камнях в куче.
//     std::vector<bool> dp(k + 1);

//     // Инициализация DP массива
//     dp[0] = false; // Нельзя сделать ход при 0 камнях.

//     // DP переход
//     for (int i = 1; i <= k; i++) { // фиксируем состояние (количество камней в куче)
//         for (int j = 0; j < n; j++) { // фиксируем переход в это состояние (выбираем элемент из множества A)
//             if (a[j] > i) {
//                 continue; // Нельзя взять элемент a[j], если его значение больше, чем камней в куче.
//             }
//             if (!dp[i - a[j]]) { // реализуем переход (проверяем, если противник проигрывает в состоянии i - a[j], то мы выигрываем в состоянии i)
//                 dp[i] = true; // Записываем, что текущий игрок побеждает в состоянии i.
//             }
//         }
//     }

//     // Отладочный вывод DP массива
//     // std::cout << "DP массив: ";
//     // for (int i = 0; i <= k; i++) {
//     //     std::cout << dp[i] << " ";
//     // }
...
//     // Выводим результат: "First", если текущий игрок побеждает в состоянии k, иначе "Second".
