...
func solve(in *bufio.Reader, out *bufio.Writer) {
	...
	var target2idx, target1idx, target0idx int = -1, -1, -1
	for i, char := range S {
		if byte(char) == Tlower[2] {
			target2idx = i
			break
		}
	}
	...
	for i, char := range S {
		if byte(char) == Tlower[0] {
			target0idx = i
			break
		}
	}

	if target0idx < target1idx && target1idx < target2idx && target0idx != -1 && target1idx != -1 && target2idx != -1 {
		fmt.Fprint(out, "Yes")
	} else if target0idx < target1idx && target2idx == -1 && target0idx != -1 && target1idx != -1 {
		fmt.Fprint(out, "Yes")
	} else {
		fmt.Fprint(out, "No")
	}
