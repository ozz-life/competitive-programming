...
void solve() {
  int N;
  cin >> N; // количество точек ломаной, задающей горную цепь (1 ≤ N ≤ 30 000)
  ...
  std::vector<int> prefix_l_to_r =
      calculateTotalAscents(y_coordinates);
  reverse(y_coordinates.begin(), y_coordinates.end());
  std::vector<int> prefix_r_to_l =
      calculateTotalAscents(y_coordinates);
  ...
  for (int i = 0; i < M; ++i) {
    ...
    cin >> l >> r;
    if (r - l >= 0)
      cout << getSum(prefix_l_to_r, l, r) << endl;
    else
      cout << getSum(prefix_r_to_l, N - l + 1, N - r + 1) << endl;
  }
}
