...
std::vector<int> buildPrefixSum(std::vector<int> &a) {
...
int getSum(std::vector<int> &pref_sum, int l, int r) {
...
void solve() {
  ...
  std::vector<int> pref_sum = buildPrefixSum(v);

  int ans = 0;
  int l = 0, r = 0;
  while (r < pref_sum.size()) {
    if (getSum(pref_sum, l, r) < K) {
      ++r;
    } else {
      if (getSum(pref_sum, l, r) == K) {
        ++ans;
