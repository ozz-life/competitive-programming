...
void solve() {
  ...
  // int n; cin >> n; // майки
  // std::vector<int> t_shirts(n);
  // for(auto& i : t_shirts) cin >> i;
  // int m; cin >> m; // штаны
  // std::vector<int> trousers(m);
  // for(auto& i : trousers) cin >> i;

  // int min_diff = std::numeric_limits<int>::max();
  // int sN, sM;
  // for (int i = 0; i < t_shirts.size(); ++i) {
  // 	for (int j = 0; j < trousers.size(); j++) {
  // 		int diff = std::abs(t_shirts[i] - trousers[j]);
  // 		if(diff < min_diff) {
  // 			min_diff = diff;
  // 			sN = t_shirts[i];
  // 			sM = trousers[j];
  ...
  //   while (i < n && j < m) {
  //     int diff = abs(t_shirts[i] - trousers[j]);
  //     if (diff < min_diff) {
  //       min_diff = diff;
  //       sN = t_shirts[i];
  //       sM = trousers[j];
