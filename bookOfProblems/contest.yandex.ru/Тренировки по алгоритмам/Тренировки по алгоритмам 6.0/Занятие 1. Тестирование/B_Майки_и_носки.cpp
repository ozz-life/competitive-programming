...
void solve() {
  ...
  if (A > 0 && C > 0) {
    ans.emplace_back(B + 1, D + 1);
  }
  if (B > 0 && D > 0) {
    ans.emplace_back(A + 1, C + 1);
  }
  if (A > 0 && B > 0) {
    ans.emplace_back(std::max(A, B) + 1, 1);
  }
  if (C > 0 && D > 0) {
    ans.emplace_back(1, std::max(C, D) + 1);
  }

  auto m = std::min_element(
      ans.begin(), ans.end(), [](const auto &lhs, const auto &rhs) {
