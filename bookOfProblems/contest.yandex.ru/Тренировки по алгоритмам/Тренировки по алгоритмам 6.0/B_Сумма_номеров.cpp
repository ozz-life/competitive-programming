...
std::vector<int64_t> MakePrefixSum(const std::vector<int64_t> &a) {
  std::vector<int64_t> ps(a.size() + 1, 0);
...
int countZeroSumRanges(const std::unordered_map<int, int>& prefixSumByValue) {
    int cntRanges = 0;

    for (const auto& entry : prefixSumByValue) {
        int cntSum = entry.second;
...
void solve() {
  ...
  std::vector<int64_t> ps = MakePrefixSum(a);
  std::unordered_map<int, int> ps_sm;
