...
func canPlaceShips(fieldLength, shipCount int) bool {
	fieldLength -= shipCount
	shipCount--
	occupiedCells := 2
	for shipCount > 0 {
		fieldLength -= (shipCount + 1) * occupiedCells
		if fieldLength < 0 {
			return false
		}
		shipCount--
...
func binSearchLastTrue(n int) int {
	l := 0
	r := n
	ans := -1
	for l <= r {
		m := (l + r) / 2
		if canPlaceShips(n, m) {
			l = m + 1
			ans = m
		} else {
			r = m - 1
...
func solve(in *bufio.Reader, out *bufio.Writer) {
	var n int
	fmt.Fscan(in, &n)
	fmt.Println(binSearchLastTrue(n))
}
