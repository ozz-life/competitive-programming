...
void solve() {
  ...
  //     if (i == j) {
  //       // Если i и j равны, разница равна 0
  //       sum += 0;
  //     } else {
  //       // В противном случае, вычислите абсолютную разницу
  //       sum += std::abs(v[i] - v[j]);
  //     }
  //      std::cout << "v[" << i << "] - v[" << j << "]: " << v[i] << " - " <<
  //      v[j] << " = " << std::abs(v[i] - v[j]) << std::endl;
  //   }
  //   ans.push_back(sum);
  ...
  std::vector<int> sums = findPrefixSums(nums);
  std::vector<int> res(n);

  res[0] = sums[n] - nums[0] * n;
  for (int i = 1; i < n; ++i) {
    res[i] = nums[i] * i - sums[i] + sums[n] - sums[i] - nums[i] * (n - i);
  }
  res[n] = nums[n] * n - sums[n];
