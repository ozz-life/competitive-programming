...
//   double angle = (angle1 - angle2) * 180 / M_PI;
...
//   double r1 = calculateRadius(x1, y1);
//   double r2 = calculateRadius(x2, y2);

//   double angle = calculateAngle(x1, y1, x2, y2);
//   double arc = calculateArcLength(min(r1, r2), angle);
//   double dif = abs(r1 - r2);

//   double res = min(r1 + r2, arc + dif);
...
struct Point {
  ...
  double dotProduct(const Point &b) const { return x * b.x + y * b.y; }

  double crossProduct(const Point &b) const { return x * b.y - y * b.x; }

  double angle(const Point &b) const {
    return atan2(crossProduct(b), dotProduct(b)) * 180 / M_PI;
  }
};
...
void solve() {
  ...
  double arc = (min(r1, r2) * angle * M_PI) / 180.0;
  double dif = abs(r1 - r2);
