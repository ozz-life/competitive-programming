...
// Возможное переполнение в (x + y)
int integerDivisionCeil(int numerator, int denominator) {
  return (numerator + denominator - 1) / denominator;
}

// Переполнение вниз на нулевом делимом. Если x = 0, y = 2
// результат будет 1, а должно быть 0
// int integerDivisionCeil(int numerator, int denominator) {
...
void solve() {
  ...
  int capacity, maxFlour;
  input >> capacity >> maxFlour;
  int time = 0;
  std::vector<int> people(maxFlour + 1, 0);

  for (int i = 1; i <= maxFlour; i++) {
    int peoples;
    input >> peoples;
    int trips = peoples / capacity;
    time += static_cast<int>(trips) * i * 2;
