...
void solve() {
    ...
    vector<vector<int>> pref_sum = findPrefixSums2D(grid);
  
    int maxSide = min(n, m); // Начнем с наибольшей возможной стороны
    int ans = 0;

    while (maxSide >= 1) {
        for (int i = 0; i <= n - maxSide; ++i) {
            for (int j = 0; j <= m - maxSide; ++j) {
                int totalSum = pref_sum[i + maxSide][j + maxSide] - pref_sum[i][j + maxSide] - pref_sum[i + maxSide][j] + pref_sum[i][j];
                if (totalSum == maxSide * maxSide) {
                    ans = max(ans, maxSide);
                }
            }
        }
        // Если нашли квадрат, завершаем поиск
        if (ans > 0) {
            break;
        }
        maxSide--; // Уменьшаем размер стороны квадрата
    }
