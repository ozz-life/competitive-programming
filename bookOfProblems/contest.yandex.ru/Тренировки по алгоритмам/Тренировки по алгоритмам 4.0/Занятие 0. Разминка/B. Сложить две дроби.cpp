...
void solve() {
  ...
  int common_denominator = b * d / gcd(b, d);
  int a_new = a * (common_denominator / b);
  int c_new = c * (common_denominator / d);

  int m = a_new + c_new;
  int g = gcd(m, common_denominator);

  int m_final = m / g;
  int n = common_denominator / g;

  std::cout << m_final << " " << n << std::endl;
}
