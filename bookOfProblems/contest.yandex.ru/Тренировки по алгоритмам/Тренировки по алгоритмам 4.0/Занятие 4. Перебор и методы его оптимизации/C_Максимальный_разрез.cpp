...
struct CutResult {
  ...
  CutResult(int w, int m) : weight(w), mask(m) {}
};

CutResult
iteratingAndFindMax(int n,
  ...
  for (int mask = 1; mask < (1 << (n - 1)); mask++) {
    ...
    // Обновление максимального веса и разреза
    if (weight > maxWeight) {
      maxWeight = weight;
      maxMask = mask;
    }
  ...
  return CutResult(maxWeight, maxMask);
}

void printHolders(int mask, int n) {
  for (int i = 0; i < n; i++) {
    int bit = (mask >> i) & 1;
    std::cout << (bit == 0 ? "1 " : "2 ");
  }
...
void solve() {
  ...
  CutResult max = iteratingAndFindMax(n, adjacencyMatrix);

  std::cout << max.weight << std::endl;
  printHolders(max.mask, n);
}
