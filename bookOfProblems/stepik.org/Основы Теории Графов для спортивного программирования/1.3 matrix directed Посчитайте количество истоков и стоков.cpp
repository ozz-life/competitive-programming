...
// Наглядно: по вертикали вершины, в которые ничего не входит, а горизонталь -
// из которых ничего не выходит.

// мы можем пройтись по каждой вершине и проверить, есть ли в соответствующей
// строке или столбце только нули. Если в строке только нули, то это исток, если
// в столбце только нули, то это сток.
pair<int, int>
countSourcesAndSinks(const vector<vector<int>> &adjacencyMatrix) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = 0; j < n; ++j) {
      if (adjacencyMatrix[i][j] != 0) {
        isSource = false; // Вершина не является истоком, если в строке есть
                          // ненулевой элемент
      }
      if (adjacencyMatrix[j][i] != 0) {
        isSink = false; // Вершина не является стоком, если в столбце есть
                        // ненулевой элемент
      }
    }
...
void solve() {
  ...
  pair<int, int> result = countSourcesAndSinks(adjacencyMatrix);
  //   cout << result.first << " " << result.second << endl;
  // код запрашивает сначала исток а потом сток, а в первом примере
  // запрашивается сток а потом исток
