...
int main() {
    ...
    // Находим первый индекс с суммой строго больше 5
    int indexWithSumGreater5 = fenwick.findIndexWithSumGreater(5);
    std::cout << "First index with sum greater than 5: " << indexWithSumGreater5 << std::endl;

    // Находим второй по порядку элемент в отсортированном списке
    int kthElement = fenwick.findKthElement(2);
    std::cout << "Second element in sorted list: " << kthElement << std::endl;

    // Находим порядковый номер элемента по индексу 4
    int orderOfElement = fenwick.findOrderOfElement(4);
    std::cout << "Order of element at index 4: " << orderOfElement << std::endl;
